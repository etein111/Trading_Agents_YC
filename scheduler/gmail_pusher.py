...
class GmailPusher:
    ...
    def send_email(self, message: EmailMessage) -> bool:
        """Send an email via Gmail SMTP. Tries port 465 (SSL) first, falls back to 587 (STARTTLS)."""
        try:
            ...
            try:
                ...
                print(f"Port {self.smtp_port} failed ({e}), trying port 587 (STARTTLS)...")
    ...
    def send_with_retry(self, message: EmailMessage, max_retries: int = 3) -> bool:
        ...
        for attempt in range(max_retries):
            ...
            if attempt < max_retries - 1:
                ...
                print(f"Retry {attempt + 1}/{max_retries} in {delays[attempt]} seconds...")
...
"""
    ...
    def format_deep_screener_report(self, report: dict, report_type: str = "screener") -> "EmailMessage":
        """Full report with per-stock TradingAgents analysis + decision layers."""
        scan_date = report.get("scan_date", "")
        type_label = {"screener": "Stock Screener", "afterhours": "After-Hours", "weekly": "Weekly Review"}.get(report_type, "Report")
    ...
    <table style="border-collapse:collapse;width:100%" border='1' cellpadding='5' cellspacing='0'>
      ...
      {sector_rows}
        ...
        # --- Per-sector stock recommendations (top 3 per sector, excluding deep stocks) ---
        deep_tickers = {s.get("ticker", "") for s in deep_stocks}
        stock_rows = ""
        for sector_name, stocks in top_by_sector.items():
            for stock_data in stocks:
                ticker = stock_data.get("ticker", "") or stock_data.get("stock", "")
                if ticker in deep_tickers:
                    continue  # skip - already in deep analysis
        ...
        # --- Per-sector recommendations (always shown, before or after positions) ---
        sector_recs_section = ""
        if stock_rows:
            sector_recs_section = f"""
    <h2 style="margin-top:30px">三、各板块推荐（不含深度分析）</h2>
    <p>以下为每板块 Top 3 推荐，不在上述深度分析列表中</p>
    <table border='1' cellpadding='5' cellspacing='0'>
      <tr><th>股票</th><th>板块</th><th>综合得分</th><th>评级</th></tr>
      {stock_rows}
    </table>"""
        ...
        <tr>
          ...
          <td>{html.escape(p.get('broker', ''))}</td>
            ...
            pos_section = f"""
    <h2 style="margin-top:30px">四、现有持仓</h2>
    <p>总持仓市值: <strong>${total_value:.2f}</strong></p>
    <table border='1' cellpadding='5' cellspacing='0'>
      ...
      {pos_rows}
        ...
        adjustments_section = f"""
    <h2 style="margin-top:30px">五、持仓调整建议</h2>
    ...
    <h1 style="color:#333">{type_label} — Agent Analysis</h1>
    <p><em>扫描日期: {scan_date} | 报告生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M')}</em></p>
    {anomaly_section}
    {sectors_table}
    <h2 style="margin-top:30px">二、Top {len(deep_stocks)} 个股深度 Agent 分析</h2>
    ...
    {pos_section}
    {adjustments_section}
    <p style="margin-top:20px"><em>Generated by TradingAgents {type_label} | 权重: A×0.4 + B×0.3 + E×0.2 + C×0.1</em></p>
    </div>"""
        return EmailMessage(
            subject=f"[TradingAgents] {type_label} — {scan_date}",
            body=body,
            to_email=self.recipient_email,
        )
